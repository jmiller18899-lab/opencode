...
- Provider credentials come from env vars named by the models.dev catalog (e.g. `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `XAI_API_KEY`). Note: this environment's secret is named `X_AI_API`, which is NOT read automatically — map it with `export XAI_API_KEY="$X_AI_API"` before starting the server/TUI.
